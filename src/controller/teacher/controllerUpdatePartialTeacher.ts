...
import { serviceUpdatePartialTeacher } from "../../services/teacher";

export const controllerUpdatePartialTeacher = async (req: Request, res: Response): Promise<void> => {
    try {
        const { data, error } = await serviceUpdatePartialTeacher(req.params.id, req.body);
        if (error) {
